from tfdiffeq import move_to_device, cast_double, func_cast_double
...
def generate_spiral2d(nspiral=1000,
    ...
    if savefig:
        ...
        plt.close()
        print('Saved ground truth spiral at {}'.format('./plots/ground_truth.png'))
...
def restore_states():
    ...
    ots = tf.convert_to_tensor(np.load(orig_ts_path), dtype=tf.float32)
...
if __name__ == '__main__':
    ...
    with tf.device(device):
        ...
        print(orig_ts.shape, orig_trajs.shape, samp_trajs.shape, samp_ts.shape)
        # model
        ...
        for itr in range(1, args.niters + 1):
            ...
            if args.visualize:
                ...
                plt.legend()
                plt.savefig('./plots/vis{}.png'.format(itr), dpi=500)
                plt.close()
                print('Saved visualization figure at {}'.format('./plots/vis{}.png'.format(itr)))
