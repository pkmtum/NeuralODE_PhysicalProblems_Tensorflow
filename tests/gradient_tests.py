...
import problems
from check_grad import gradcheck
...
class TestGradient(unittest.TestCase):
    ...
    def test_adjoint(self):
        ...
        t_points = tf.cast(t_points, tf.float32)
        ...
        print(adj_t_grad, adj_a_grad, adj_b_grad, reg_t_grad, reg_a_grad, reg_b_grad)
        print((reg_t_grad - adj_t_grad)/reg_t_grad, tf.norm(reg_t_grad), tf.norm(adj_t_grad))
        print((reg_a_grad - adj_a_grad)/reg_a_grad, tf.norm(reg_a_grad), tf.norm(adj_a_grad))
        print((reg_b_grad - adj_b_grad)/reg_b_grad, tf.norm(reg_b_grad), tf.norm(adj_b_grad))
        print(max_abs(reg_t_grad - adj_t_grad))
        print(max_abs(reg_a_grad - adj_a_grad))
        print(max_abs(reg_b_grad - adj_b_grad))
...
class TestCompareAdjointGradient(unittest.TestCase):
    ...
    def problem(self):
        ...
        class Odefunc(tf.keras.Model):
            ...
            def __init__(self):
                ...
                self.A = tf.Variable([[-0.1, -2.0], [2.0, -0.1]], dtype=tf.float32)
                self.unused_module = tf.keras.layers.Dense(5, dtype=tf.float32)
                self.unused_module.build((5,))

            def call(self, t, y):
                return tf.linalg.matvec(self.A, y ** 3)

        y0 = tf.convert_to_tensor([2., 0.], dtype=tf.float32)
        t_points = tf.linspace(
            tf.constant(0., dtype=tf.float32),
            tf.constant(25., dtype=tf.float32),
            10
        )
        func = Odefunc()
    ...
    def test_dopri5_adjoint_against_dopri5(self):
        ...
        gradys = 0.1 * tf.random.uniform(shape=ys.shape, dtype=tf.float32)
