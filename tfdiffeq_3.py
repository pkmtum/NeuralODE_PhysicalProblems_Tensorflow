...
def create_spirals(N):
    """ Creates a list of points which form a spiral
        # Arguments:
            N: Integers, number of points to be generated
        # Output:
...
# class ODEFunc(tf.keras.Model):
    ...
    """
    def __init__(self, hidden_dim, time_dependent=False, activation='relu',
                 **kwargs):
        super(ODEFunc, self).__init__(**kwargs, dtype=tf.float64)
        self.hidden_dim = hidden_dim
        self.nfe = 0
        self.time_dependent = time_dependent

        self.fc1 = Dense(hidden_dim, activation=activation)
        self.fc2 = Dense(hidden_dim, activation=activation)

    def build(self, input_shape):
        if len(input_shape) > 0:
            self.fc3 = Dense(input_shape[-1])
            self.built = True
    ...
    def call(self, t, x, training=None, **kwargs):
        ...
        tf.print(t, x)
        tf.print(t.shape, x.shape, self.nfe)


        if self.fc3 is None:
            self.fc3 = Dense(x.shape.as_list()[-1])
        self.nfe += 1
        if self.time_dependent:
            t_vec = tf.ones([x.shape[0], 1], dtype=t.dtype) * t
            t_and_x = tf.concat([t_vec, x], axis=-1)
            out = self.fc1(tf.cast(t_and_x, dtype=tf.float32))
        else:
            out = self.fc1(x)
        out = self.fc2(x)
        out = self.fc3(x)
...
t = tf.cast(tf.linspace(0., 1., 3), dtype=tf.float32)
odemodel = ODEFunc(hidden_dim=16)

x, y = create_spirals(1200)
...
with tf.device('/gpu:0'):
    with tf.GradientTape(persistent=True) as g:
        for i in range(10):
            batch = tf.constant(x_train[i*16:(i+1)*16])
            y = odeint_adjoint(odemodel, batch, t)
            y = y[-1]
